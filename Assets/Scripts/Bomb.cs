...
public class Bomb : MonoBehaviour
{
    ...
    void Explode()
    {
        ...
        {
            Instantiate(Constants.hasBombExpander ? explosionRadio : explosionRadioEdge, new Vector3(transform.position.x + 1, transform.position.y, 0), Quaternion.identity);

            if (Constants.hasBombExpander && transform.position.x + 2 < Constants.WorldEndX)
        ...
        {
            Instantiate(Constants.hasBombExpander ? explosionRadio : explosionRadioEdge, new Vector3(transform.position.x - 1, transform.position.y, 0), Quaternion.Euler(0, 0, 180));

            if (Constants.hasBombExpander && transform.position.x - 2 > Constants.WorldBeginX)
        ...
        {
            Instantiate(Constants.hasBombExpander ? explosionRadio : explosionRadioEdge, new Vector3(transform.position.x, transform.position.y + 1, 0), Quaternion.Euler(0, 0, 90));

            if (Constants.hasBombExpander && transform.position.y + 2 < Constants.WorldBeginY)
        ...
        if (transform.position.y - 1 > Constants.WorldEndY && !blockVertical)
        {
            ...
            if (Constants.hasBombExpander && transform.position.y - 2 > Constants.WorldEndY)
                Instantiate(explosionRadioEdge, new Vector3(transform.position.x, transform.position.y - 2, 0), Quaternion.Euler(0, 0, -90));
        }
